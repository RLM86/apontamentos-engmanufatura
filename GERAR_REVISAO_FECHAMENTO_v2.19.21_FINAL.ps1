...
function Replace-Regex-Required {
  ...
  $Matches = $Regex.Matches($TextNorm)

  if ($Matches.Count -eq 0) {
    throw "Padrao obrigatorio nao encontrado: $Label."
  }

  if ($Matches.Count -gt 1) {
    throw "Padrao encontrado $($Matches.Count) vezes: $Label. Processo interrompido para evitar alterar local errado."
  }
  ...
  Write-Host ("OK - " + $Label) -ForegroundColor DarkGreen
  return $Regex.Replace($TextNorm, $Evaluator, 1)
}
...
if (-not $app.Contains('planned_detail:plannedHoursDetail(planned)')) {
  throw "app-v2.19.20.js nao contem o bloco de fechamento esperado."
}
if (-not $app.Contains('data-toggle-closing-details')) {
  throw "app-v2.19.20.js nao contem a tabela Fechamentos enviados esperada."
}
...
$app = Replace-Regex-Required `
  $app `
  '(?m)^\s*const\s+conference\s*=\s*closingConference\(balance\);\s*\n\s*return\s*\{' `
  $conferenceReplacement `
...
$app = Replace-Regex-Required `
  $app `
  '(?m)^\s*planned_detail\s*:\s*plannedHoursDetail\(planned\),\s*\n\s*project_ids\s*:\s*projectIds,' `
  $plannedDetailReplacement `
...
$app = Replace-Regex-Required `
  $app `
  '(?m)^\s*const\s+selectorCell\s*=\s*isManager\(\)' `
  $selectorReplacement `
...
$app = Replace-Regex-Required `
  $app `
  '(?m)^\s*</div>\s*\n\s*<div\s+class="closing-compact-details-actions">' `
  $detailsActionsReplacement `
...
$app = Replace-Regex-Required `
  $app `
  '(?m)^\s*const\s+button\s*=\s*event\.target\.closest\("\[data-open-closing\]"\);' `
  $clickReplacement `
  "acao ajustar dia"

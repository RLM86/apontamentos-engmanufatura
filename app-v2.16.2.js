(() => {
  ...
  $("projectRoomsTable").onclick=async e=>{const button=e.target.closest("button");if(!button)return;const editId=button.dataset.editRoomInstance,toggleId=button.dataset.toggleRoomInstance,deleteId=button.dataset.deleteRoomInstance;
    ...
    if(deleteId&&confirm(`Excluir ${roomInstanceName(deleteId)}?`)){showLoading(true);try{await deleteRoomInstance(deleteId);toast("Sala excluída.");await reloadCatalogs()}catch(error){handleError(error,"Sala utilizada em apontamentos deve ser inativada.")}finally{showLoading(false)}}
  };
  ...
  if("serviceWorker" in navigator && location.protocol.startsWith("http")){
    navigator.serviceWorker
      .register("sw.js?v=2.16.2", {updateViaCache:"none"})
  ...
  window.APONTA_P3_VERSION = "2.16.2";
})();

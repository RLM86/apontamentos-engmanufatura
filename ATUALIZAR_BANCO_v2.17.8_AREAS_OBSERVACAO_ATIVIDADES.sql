-- APONTA P3 v2.17.8
-- EDIÇÃO TRANSACIONAL DAS ÁREAS E OBRIGATORIEDADE DA OBSERVAÇÃO
-- Execute todo este arquivo no SQL Editor do Supabase.

begin;

create or replace function public.aponta_save_activity_v2178(
  p_activity_id uuid,
  p_code text,
  p_name text,
...
begin
  ...
  select area_code
    into v_invalid_area
  from unnest(p_areas) as area_code
  where not exists (
    select 1
    from public.work_areas wa
    where wa.code = area_code
  )
  limit 1;
  ...
  insert into public.activity_area_links(
    activity_id,
    area_code
  )
  select
  ...
  return jsonb_build_object(
    ...
    'created', v_created,
    'observation_requirement', v_observation,
    'areas', to_jsonb(p_areas)
  );
end;
$$;

revoke all on function public.aponta_save_activity_v2178(
  uuid,
  text,
  text,
  text,
...
grant execute on function public.aponta_save_activity_v2178(
  uuid,
  text,
  text,
  text,
...
create or replace function public.aponta_import_activities_v2178(
  p_rows jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  loop
    begin
      ...
      if jsonb_typeof(v_areas_json) <> 'array'
         or jsonb_array_length(v_areas_json) = 0
      then
      ...
      select array_agg(value)
        into v_areas
      from jsonb_array_elements_text(v_areas_json);
      ...
      v_result := public.aponta_save_activity_v2178(
        v_activity_id,
        v_code,
        v_name,
        v_discipline_name,
...
revoke all on function public.aponta_import_activities_v2178(jsonb)
from public;

grant execute on function public.aponta_import_activities_v2178(jsonb)
to authenticated;
...
where routine_schema = 'public'
  and routine_name in (
    'aponta_save_activity_v2178',
    'aponta_import_activities_v2178'
  )

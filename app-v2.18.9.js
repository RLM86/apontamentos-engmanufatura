(() => {
  ...
  async function renderClosingHistory(){
    ...
    table.innerHTML='<tr><td colspan="9" class="empty">Carregando fechamentos...</td></tr>';
    ...
    try{
      ...
      }).join("")||'<tr><td colspan="9" class="empty">Nenhum fechamento enviado encontrado com os filtros selecionados.</td></tr>';
    }catch(error){
      closingHistoryRows=[];
      updateClosingHistorySummary([]);
      table.innerHTML='<tr><td colspan="9" class="empty">Não foi possível carregar os fechamentos.</td></tr>';
  ...
  if($("filterClosingHistoryBtn")) $("filterClosingHistoryBtn").onclick=renderClosingHistory;
  if($("closingHistorySearch")){
    $("closingHistorySearch").addEventListener("keydown",event=>{
      if(event.key==="Enter")renderClosingHistory();
    });
  }
  ...
  if("serviceWorker" in navigator && location.protocol.startsWith("http")){
    navigator.serviceWorker
      .register("sw.js?v=2.18.9", {updateViaCache:"none"})
  ...
  window.APONTA_P3_VERSION = "2.18.9";
})();

...
`+
  ...
  $("exportReportBtn").onclick=()=>{
    const lines=[["Data","Colaborador","Projeto","Área","Referência","Atividade","Disciplina","Horas","Observação","Status"],...lastReportRows.map(x=>[x.entry_date,profileName(x.user_id),projectName(x.project_id),areaName(x.area_code),entryReferenceName(x),activityName(x.activity_id),activityDiscipline(x.activity_id),String(x.hours).replace(".",","),x.details,statusLabel(x.status)])];

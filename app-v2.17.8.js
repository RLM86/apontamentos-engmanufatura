(() => {
  ...
  $("activityForm").onsubmit=async e=>{
    ...
    try{
      ...
      handleError(
        error,
        "Não foi possível salvar a atividade. Execute o SQL obrigatório da versão 2.17.8."
      );
    }finally{
      showLoading(false);
    }
  ...
  async function renderCatalogs(){
    ...
    visibleActivityIds=activities.map(activity=>activity.id);
    const existingActivityIds=new Set(visibleActivityIds);
    Array.from(selectedActivityIds).forEach(id=>{
      if(!existingActivityIds.has(id))selectedActivityIds.delete(id);
    });

    $("activitiesTable").innerHTML=activities.map(x=>`<tr>
    ...
    </tr>`).join("")||`<tr><td colspan="${isAdmin()?8:7}" class="empty">Nenhuma atividade cadastrada</td></tr>`;
  ...
  $("saveActivityEditBtn").onclick=async e=>{
    ...
    try{
      ...
      toast("Atividade e áreas atualizadas.");
      await reloadCatalogs();
    }catch(error){
      handleError(
        error,
        "Não foi possível atualizar a atividade. Execute o SQL obrigatório da versão 2.17.8."
      );
    }finally{
      showLoading(false);
    }
  ...
  if("serviceWorker" in navigator && location.protocol.startsWith("http")){
    navigator.serviceWorker
      .register("sw.js?v=2.17.8", {updateViaCache:"none"})
  ...
  window.APONTA_P3_VERSION = "2.17.8";
})();

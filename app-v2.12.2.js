(() => {
  ...
  function moduleName(id, projectId = "") {
    const link = projectModules.find(row => row.project_id === projectId && row.module_id === id);
    return link?.display_name || modules.find(row => row.id === id)?.name || "—";
  }
  function roomName(id) {
    return rooms.find(row => row.id === id)?.name || "—";
  }
  ...
  function entryReferenceName(entry) {
    ...
    if (entry.area_code === "MES") return moduleName(entry.module_id, entry.project_id);
    if (entry.area_code === "MPA") return panelTypeName(entry.panel_type_id);
    if (entry.area_code === "MFI") return roomName(entry.room_id);
  ...
  async function loadBaseData() {
    const [p, pr, ac, ho, wa, ms, mo, ro, pt, aal, pm] = await Promise.all([
    ...
    for (const result of [wa,ms,mo,ro,pt,aal,pm]) {
      if (result.error) throw new Error("Fluxo de áreas não instalado. Execute ATUALIZAR_BANCO_v2.12_FLUXO_AREAS.sql no Supabase.");
    }
    ...
    rooms=ro.data||[]; panelTypes=pt.data||[]; activityAreaLinks=aal.data||[]; projectModules=pm.data||[];
  ...
  function projectModuleOptions(projectId) {
    const links = projectModules.filter(link => link.project_id === projectId && link.active !== false);
    const rows = links.map(link => ({
      id: link.module_id,
      name: link.display_name || modules.find(module => module.id === link.module_id)?.name || "Módulo"
    }));
    if (rows.length) return rows;
    return activeRows(modules).map(module => ({id:module.id,name:module.name}));
  }

  function setConditionalField(prefix, areaCode, selectedValue = "") {
    const configs = {
      FAB: {suffix:"Sector", rows:activeRows(manufacturingSectors), placeholder:"Selecione o setor"},
      MES: {suffix:"Module", rows:projectModuleOptions($(prefix+"Project").value), placeholder:"Selecione o módulo"},
      MPA: {suffix:"PanelType", rows:activeRows(panelTypes), placeholder:"Selecione o tipo de painel"},
      MFI: {suffix:"Room", rows:activeRows(rooms), placeholder:"Selecione a sala"}
    };
    ["Sector","Module","PanelType","Room"].forEach(suffix => {
      const field=$(prefix+suffix+"Field");
      const select=$(prefix+suffix);
      if (!field || !select) return;
      const show=configs[areaCode]?.suffix===suffix;
      field.hidden=!show;
      select.required=show;
      if (!show) select.value="";
    });
    const config=configs[areaCode];
    if (!config) return;
    const select=$(prefix+config.suffix);
    select.innerHTML=optionsWithPlaceholder(config.rows,row=>row.name,row=>row.id,config.placeholder);
  ...
  function refreshEntryFlow(prefix, selected = {}) {
    const areaCode=$(prefix+"Area").value;
    const detailValue=areaCode==="FAB"?selected.sector_id:areaCode==="MES"?selected.module_id:areaCode==="MPA"?selected.panel_type_id:areaCode==="MFI"?selected.room_id:"";
    setConditionalField(prefix,areaCode,detailValue||"");
  ...
  function entryFlowPayload(prefix) {
    ...
    return {
      ...
      room_id:areaCode==="MFI"?$(prefix+"Room").value:null
    };
  }

  function validateEntryFlow(prefix) {
    const area=$(prefix+"Area").value;
    if (!area) { toast("Selecione a área do apontamento.",true); $(prefix+"Area").focus(); return false; }
    const config={FAB:"Sector",MES:"Module",MPA:"PanelType",MFI:"Room"}[area];
    if (config && !$(prefix+config).value) { toast("Preencha o campo específico da área selecionada.",true); $(prefix+config).focus(); return false; }
    if (!$(prefix+"Activity").value) { toast("Selecione a atividade.",true); $(prefix+"Activity").focus(); return false; }
    if ($(prefix+"Details").required && !$(prefix+"Details").value.trim()) { toast("A observação é obrigatória para esta atividade.",true); $(prefix+"Details").focus(); return false; }
    return true;
  }
  ...
  function fillSelects() {
    ...
    $("projectModuleProject").innerHTML=optionsWithPlaceholder(activeRows(projects),row=>row.code?`${row.code} — ${row.name}`:row.name,row=>row.id,"Selecione o projeto");
  ...
  function switchCatalogView(view){
    const selectedView=view==="structure"?"structure":"general";
    ...
    const generalPanel=$("generalCatalogPanel");
    const structurePanel=$("structureCatalogPanel");

    if(generalPanel)generalPanel.hidden=selectedView!=="general";
    if(structurePanel)structurePanel.hidden=selectedView!=="structure";
  ...
  async function renderCatalogs(){
    ...
    $("projectModulesTable").innerHTML=projectModules.map(link=>`<tr><td>${esc(projectName(link.project_id))}</td><td>${esc(moduleName(link.module_id,link.project_id))}</td><td>${link.active?"Ativo":"Inativo"}</td><td><button class="btn secondary small" data-toggle-project-module-project="${link.project_id}" data-toggle-project-module-module="${link.module_id}" data-active="${link.active}">${link.active?"Inativar":"Ativar"}</button></td></tr>`).join("");
  }
  ...
  $("roomForm").onsubmit=async e=>{e.preventDefault();const{error}=await sb.from("rooms").insert({code:$("roomCode").value.trim().toUpperCase(),name:$("roomName").value.trim()});if(error)handleError(error);else{$("roomForm").reset();toast("Sala adicionada.");await reloadCatalogs()}};
  $("projectModuleForm").onsubmit=async e=>{e.preventDefault();showLoading(true);try{const code=$("moduleCode").value.trim().toUpperCase();const name=$("moduleName").value.trim();let module=modules.find(row=>normalizeText(row.code)===normalizeText(code));if(!module){const{data,error}=await sb.from("modules").insert({code,name}).select("*").single();if(error)throw error;module=data;}const{error:linkError}=await sb.from("project_modules").upsert({project_id:$("projectModuleProject").value,module_id:module.id,display_name:name,active:true},{onConflict:"project_id,module_id"});if(linkError)throw linkError;$("projectModuleForm").reset();toast("Módulo vinculado ao projeto.");await reloadCatalogs()}catch(error){handleError(error)}finally{showLoading(false)}};
  ...
  $("projectModulesTable").onclick=async e=>{const projectId=e.target.dataset.toggleProjectModuleProject;const moduleId=e.target.dataset.toggleProjectModuleModule;if(!projectId||!moduleId)return;const{error}=await sb.from("project_modules").update({active:e.target.dataset.active!=="true"}).eq("project_id",projectId).eq("module_id",moduleId);if(error)handleError(error);else reloadCatalogs()};
  ...
  window.APONTA_P3_VERSION = "2.12.2";
})();

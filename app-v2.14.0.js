(() => {
  ...
  function fillProjectStructureSelects() {
    ...
    ["projectRoomProject","projectRoomModuleProject"].forEach(id=>{
      const select=$(id);
      if (!select) return;
    ...
    const roomCatalogSelect=$("projectRoomRoom");
    if (roomCatalogSelect) {
      const old=roomCatalogSelect.value;
      roomCatalogSelect.innerHTML=optionsWithPlaceholder(
        activeRows(rooms),row=>row.name,row=>row.id,"Selecione a sala"
      );
      if (old) roomCatalogSelect.value=old;
    }
  ...
  function refreshProjectRoomModuleRoomSelect() {
    ...
    const rows=projectRoomOptions(projectSelect.value);
    roomSelect.innerHTML=optionsWithPlaceholder(
  ...
  function switchCatalogView(view){
    const allowed=new Set(["general","references","projects"]);
  ...
  async function syncProjectRoomModuleQuantity(projectId,roomId,desiredValue){
    const desired=Math.max(0,Math.min(200,Number(desiredValue||0)));
  ...
  async function renderCatalogs(){
    ...
    $("projectRoomsCount").textContent=`${projectRooms.length} vínculos`;
    $("projectRoomsTable").innerHTML=projectRooms.slice().sort((a,b)=>projectName(a.project_id).localeCompare(projectName(b.project_id),"pt-BR")||(a.order_index||0)-(b.order_index||0)).map(link=>`<tr>
      ...
      <td><div class="table-actions">${admin?`<button class="btn primary small" data-edit-project-room-project="${link.project_id}" data-edit-project-room-room="${link.room_id}">Editar</button>`:""}<button class="btn secondary small" data-toggle-project-room-project="${link.project_id}" data-toggle-project-room-room="${link.room_id}" data-active="${link.active}">${link.active?"Inativar":"Ativar"}</button></div></td>
    </tr>`).join("")||'<tr><td colspan="5" class="empty">Nenhuma sala vinculada aos projetos</td></tr>';

    $("projectRoomModulesCount").textContent=`${projectRoomModules.length} vínculos`;
    $("projectRoomModulesTable").innerHTML=projectRoomModules.slice().sort((a,b)=>projectName(a.project_id).localeCompare(projectName(b.project_id),"pt-BR")||roomName(a.room_id,a.project_id).localeCompare(roomName(b.room_id,b.project_id),"pt-BR")||(a.order_index||0)-(b.order_index||0)).map(link=>{
      ...
      return `<tr>
        ...
        <td><div class="table-actions">${admin?`<button class="btn primary small" data-edit-project-room-module-project="${link.project_id}" data-edit-project-room-module-room="${link.room_id}" data-edit-project-room-module-module="${link.module_id}">Editar</button>`:""}<button class="btn secondary small" data-toggle-project-room-module-project="${link.project_id}" data-toggle-project-room-module-room="${link.room_id}" data-toggle-project-room-module-module="${link.module_id}" data-active="${link.active}">${link.active?"Inativar":"Ativar"}</button></div></td>
      </tr>`;
    }).join("")||'<tr><td colspan="7" class="empty">Nenhum módulo vinculado às salas</td></tr>';
  ...
  $("projectRoomForm").onsubmit=async e=>{
    e.preventDefault();showLoading(true);
    try{
      const projectId=$("projectRoomProject").value;
      const roomId=$("projectRoomRoom").value;
      const catalogRoom=rooms.find(row=>row.id===roomId);
      const displayName=$("projectRoomDisplayName").value.trim()||catalogRoom?.name||"Sala";
      const quantity=Number($("projectRoomModuleQuantity").value||0);
      const{error}=await sb.from("project_rooms").upsert({project_id:projectId,room_id:roomId,display_name:displayName,order_index:Number($("projectRoomOrder").value||0),active:true},{onConflict:"project_id,room_id"});
      if(error)throw error;
      await syncProjectRoomModuleQuantity(projectId,roomId,quantity);
      $("projectRoomForm").reset();
      $("projectRoomModuleQuantity").value="1";
      $("projectRoomOrder").value="0";
      toast("Sala e quantidade de módulos salvas no projeto.");
  ...
  $("saveProjectRoomEditBtn").onclick=async e=>{e.preventDefault();showLoading(true);try{
    ...
    await syncProjectRoomModuleQuantity(projectId,roomId,Number($("editProjectRoomModuleQuantity").value||0));
  ...
  window.APONTA_P3_VERSION = "2.14.0";
})();

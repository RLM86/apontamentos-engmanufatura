(() => {
  ...
  async function loadBaseData() {
    const [p, pr, ac, ho, wa, ms, mo, ro, pt, aal, pm, pro, prm, pri, prim] = await Promise.all([
      ...
      sb.rpc("aponta_list_activity_area_links_v2181"),
    ...
    if(aal.error){
      throw new Error(
        "Não foi possível carregar as áreas vinculadas às atividades. " +
        "Execute o SQL obrigatório da versão 2.18.1 no Supabase."
      );
    }
  ...
  $("activityForm").onsubmit=async e=>{
    ...
    try{
      ...
      handleError(
        error,
        "Não foi possível salvar a atividade. Execute o SQL obrigatório da versão 2.18.1."
      );
    }finally{
      showLoading(false);
    }
  ...
  $("saveActivityEditBtn").onclick=async e=>{
    ...
    try{
      ...
      handleError(
        error,
        "Não foi possível atualizar a atividade. Execute o SQL obrigatório da versão 2.18.1."
      );
    }finally{
      showLoading(false);
    }
    ...
    try{
      const {error}=await sb.rpc(
        "aponta_save_work_area_v2182",
        {
          ...
          p_detail_type:$("workAreaDetailType").value,
      ...
      handleError(
        error,
        "Não foi possível cadastrar a área. Execute o SQL obrigatório da versão 2.18.2."
      );
    }finally{
      showLoading(false);
    }
  ...
  $("workAreasTable").onclick=async e=>{
    ...
    if(editCode){
      ...
      $("editWorkAreaDetailType").value=area.detail_type||"none";
      ...
      try{
        ...
        const {error}=await sb.rpc(
          "aponta_save_work_area_v2182",
          {
            ...
            p_code:area.code,
            p_name:area.name,
            p_detail_type:area.detail_type||"none",
    ...
    try{
      ...
      const {error}=await sb.rpc(
        "aponta_save_work_area_v2182",
        {
          ...
          p_detail_type:$("editWorkAreaDetailType").value,
      ...
      handleError(
        error,
        "Não foi possível atualizar a área. Execute o SQL obrigatório da versão 2.18.2."
      );
    }finally{
      showLoading(false);
    }
  ...
  if("serviceWorker" in navigator && location.protocol.startsWith("http")){
    navigator.serviceWorker
      .register("sw.js?v=2.18.2", {updateViaCache:"none"})
  ...
  window.APONTA_P3_VERSION = "2.18.2";
})();

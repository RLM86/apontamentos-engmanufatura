(() => {
  ...
  $("activityForm").onsubmit=async e=>{
    ...
    try{
      ...
      const persistedAreas=activityAreas(data.id);
      const missingAreas=areas.filter(code=>!persistedAreas.includes(code));
      if(missingAreas.length){
        throw new Error(`As áreas ${missingAreas.join(", ")} não foram confirmadas pelo banco.`);
      }

      toast(`Atividade adicionada com ${persistedAreas.length} área${persistedAreas.length===1?"":"s"}.`);
    }catch(error){
      handleError(
        error,
        "Não foi possível salvar a atividade. Execute o SQL obrigatório da versão 2.18.0."
      );
    }finally{
      showLoading(false);
    }
  ...
  $("saveActivityEditBtn").onclick=async e=>{
    ...
    try{
      ...
      await reloadCatalogs();
      const persistedAreas=activityAreas(id);
      const missingAreas=areas.filter(code=>!persistedAreas.includes(code));
      const unexpectedAreas=persistedAreas.filter(code=>!areas.includes(code));

      if(missingAreas.length||unexpectedAreas.length){
        throw new Error("As áreas exibidas não correspondem às áreas selecionadas. Atualize a tela e tente novamente.");
      }

      $("editActivityDialog").close();
      toast(`Atividade atualizada com ${persistedAreas.length} área${persistedAreas.length===1?"":"s"}.`);
    }catch(error){
      handleError(
        error,
        "Não foi possível atualizar a atividade. Execute o SQL obrigatório da versão 2.18.0."
      );
    }finally{
      showLoading(false);
    }
  ...
  if("serviceWorker" in navigator && location.protocol.startsWith("http")){
    navigator.serviceWorker
      .register("sw.js?v=2.18.0", {updateViaCache:"none"})
  ...
  window.APONTA_P3_VERSION = "2.18.0";
})();

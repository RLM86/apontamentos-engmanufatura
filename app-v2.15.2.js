(() => {
  ...
  function entryReferenceName(entry) {
    ...
    if (entry.area_code === "MES") {
      const room = roomName(entry.room_id, entry.project_id);
      const module = moduleName(entry.module_id, entry.project_id, entry.room_id);
      return `${room} / ${module}`;
    }
    if (entry.area_code === "MPA") return panelTypeName(entry.panel_type_id);
    if (entry.area_code === "MFI") return roomName(entry.room_id, entry.project_id);
  ...
  async function loadBaseData() {
    const [p, pr, ac, ho, wa, ms, mo, ro, pt, aal, pm, pro, prm] = await Promise.all([
    ...
    for (const result of [wa,ms,mo,ro,pt,aal,pm,pro,prm]) {
      if (result.error) {
        throw new Error(
          "Estrutura Projeto > Sala > Módulo não instalada. " +
          "Execute ATUALIZAR_BANCO_v2.13_ESTRUTURA_SALA_MODULO.sql no Supabase."
        );
      }
    }
    profiles=p.data||[]; projects=pr.data||[]; activities=ac.data||[]; holidays=ho.data||[];
  ...
  function projectRoomOptions(projectId) {
    const links = projectRooms
      .filter(link => link.project_id === projectId && link.active !== false)
      .sort((a,b)=>(a.order_index||0)-(b.order_index||0));
    return links.map(link => ({
      id:link.room_id,
      name:link.display_name ||
        rooms.find(room => room.id === link.room_id)?.name ||
        "Sala"
    }));
  }

  function projectRoomModuleOptions(projectId, roomId) {
    if (!projectId || !roomId) return [];
    const links = projectRoomModules
      .filter(link =>
    ...
    return links.map(link => ({
      id:link.module_id,
      name:link.display_name ||
        modules.find(module => module.id === link.module_id)?.name ||
        "Módulo"
    }));
  }
  ...
  function setEntryStepNumbers(prefix, areaCode) {
    ...
    const activityStep = $("entryActivityStep");
    ...
    if (activityStep) activityStep.textContent = structural ? "5" : noDetail ? "3" : "4";
  }

  function refreshStructuralModule(prefix, selectedModuleId = "") {
    const projectId=$(prefix+"Project").value;
    const roomId=$(prefix+"Room").value;
    const select=$(prefix+"Module");
    if (!select) return;
    const rows=projectRoomModuleOptions(projectId,roomId);
    select.innerHTML=optionsWithPlaceholder(
      rows,
      row=>row.name,
      row=>row.id,
      roomId ? "Selecione o módulo da sala" : "Selecione primeiro a sala"
    );
    select.disabled=!roomId;
  ...
  function setConditionalField(prefix, areaCode, selected = {}) {
    ...
    if ($(prefix+"RoomLabel")) {
      $(prefix+"RoomLabel").textContent=
        areaCode==="MES" ? "Sala da montagem estrutural" : "Sala da montagem final";
    }
    ...
    if (showSector) {
      ...
      select.innerHTML=optionsWithPlaceholder(
        ...
        row=>row.name,row=>row.id,"Selecione o setor"
      );
    ...
    if (showPanel) {
      ...
      select.innerHTML=optionsWithPlaceholder(
        ...
        row=>row.name,row=>row.id,"Selecione o tipo de painel"
      );
    ...
    if (showRoom) {
      ...
      roomSelect.innerHTML=optionsWithPlaceholder(
        roomRows,
        row=>row.name,row=>row.id,
        projectId ? "Selecione a sala do projeto" : "Selecione primeiro o projeto"
      );
      roomSelect.disabled=!projectId;
      if (selected.room_id) roomSelect.value=selected.room_id;
    }

    if (showModule) {
      refreshStructuralModule(prefix,selected.module_id||"");
    }
  ...
  function updateActivityHelp(prefix) {
    ...
    if (!activity) {
      ...
      if (rule) rule.textContent="Recomendada";
      return;
    }
    const required=normalizeText(activity.observation_requirement)==="sim" || normalizeText(activity.observation_requirement).includes("obrig");
    details.required=required;
    rule.textContent=required?"Obrigatória":"Recomendada";
  ...
  function entryFlowPayload(prefix) {
    ...
    return {
      ...
      room_id:["MES","MFI"].includes(areaCode)?$(prefix+"Room").value:null,
      module_id:areaCode==="MES"?$(prefix+"Module").value:null,
  ...
  function validateEntryFlow(prefix) {
    ...
    if ($(prefix+"Details").required && !$(prefix+"Details").value.trim()) {
  ...
  function fillProjectStructureSelects() {
    const projectOptions=optionsWithPlaceholder(
      activeRows(projects),
      row=>row.code?`${row.code} — ${row.name}`:row.name,
      row=>row.id,
      "Selecione o projeto"
    );
    ...
    const compositionSelect=$("projectCompositionProject");
    if(compositionSelect&&!compositionSelect.value&&activeRows(projects).length){
      compositionSelect.value=activeRows(projects)[0].id;
    }

    const advancedProject=$("projectRoomModuleProject");
    if(advancedProject&&compositionSelect?.value){
      advancedProject.value=compositionSelect.value;
    }
  ...
  function refreshProjectRoomModuleRoomSelect() {
    const projectSelect=$("projectRoomModuleProject");
    const roomSelect=$("projectRoomModuleRoom");
    if(!projectSelect||!roomSelect)return;
    const old=roomSelect.value;
    const rows=projectRoomOptions(projectSelect.value).filter(row=>row.active!==false);
    roomSelect.innerHTML=optionsWithPlaceholder(
      rows,row=>row.name,row=>row.id,
      projectSelect.value?"Selecione a sala do projeto":"Selecione primeiro o projeto"
    );
  ...
  $("entryRoom").onchange=()=>{
    if ($("entryArea").value==="MES") refreshStructuralModule("entry");
  };
  ...
  $("editEntryRoom").onchange=()=>{
    if ($("editEntryArea").value==="MES") refreshStructuralModule("editEntry");
  };
    ...
    try {
      ...
      $("entriesTable").innerHTML = rows.map(x => {
        ...
        return `<tr>
          ...
          <td>${esc(projectName(x.project_id))}</td><td>${esc(areaName(x.area_code))}</td>
          <td>${esc(entryReferenceName(x))}</td><td>${esc(activityName(x.activity_id))}</td>
      ...
      }).join("") || '<tr><td colspan="9" class="empty">Nenhum apontamento encontrado</td></tr>';
      await updateDayTotal();
    } catch (e) { handleError(e); }
  }
  ...
  $("copyPreviousBtn").onclick = async () => {
    ...
    try {
      ...
      const copies=source.map(x=>({user_id:userId,entry_date:target,project_id:x.project_id,activity_id:x.activity_id,area_code:x.area_code,sector_id:x.sector_id,module_id:x.module_id,room_id:x.room_id,panel_type_id:x.panel_type_id,hours:x.hours,details:x.details,status:"rascunho"}));
  ...
  $("activityForm").onsubmit=async e=>{
    ...
    const payload={code:$("activityCode").value.trim()||null,name:$("activityName").value.trim(),discipline_name:$("activityDiscipline").value.trim(),nature:$("activityNature").value.trim(),usage_description:$("activityUsageDescription").value.trim(),observation_requirement:$("activityObservationRequirement").value,sector_principal:areas.map(areaName).join(" / "),activity_type:"Demanda",frequency:"",responsible_name:"",backup_name:"",created_by:me.id};
  ...
  function activeProjectRoomModuleCount(projectId,roomId){
    return projectRoomModules.filter(link=>
      link.project_id===projectId&&link.room_id===roomId&&link.active
    ).length;
  }
  ...
  async function syncProjectRoomModuleQuantity(projectId,roomId,desiredValue){
    const numericValue=Number(desiredValue||0);
    if(!Number.isFinite(numericValue)||numericValue<0||!Number.isInteger(numericValue)){
      throw new Error("A quantidade de módulos deve ser um número inteiro igual ou maior que zero.");
    }
    const desired=numericValue;
    const project=projects.find(row=>row.id===projectId);
    const room=rooms.find(row=>row.id===roomId);
    if(!project||!room)throw new Error("Projeto ou sala não encontrados.");

    let links=projectRoomModules
      .filter(link=>link.project_id===projectId&&link.room_id===roomId)
      .slice().sort((a,b)=>(a.order_index||0)-(b.order_index||0));
    let activeLinks=links.filter(link=>link.active);
    let inactiveLinks=links.filter(link=>!link.active);
    let nextOrder=activeLinks.reduce((max,row)=>Math.max(max,Number(row.order_index||0)),0)+1;

    while(activeLinks.length<desired&&inactiveLinks.length){
      const link=inactiveLinks.shift();
      const order=nextOrder++;
      const{error}=await sb.from("project_room_modules").update({active:true,order_index:order})
        .eq("project_id",projectId).eq("room_id",roomId).eq("module_id",link.module_id);
      if(error)throw error;
      await sb.from("project_modules").upsert({project_id:projectId,module_id:link.module_id,display_name:link.display_name||moduleName(link.module_id,projectId,roomId),order_index:order,active:true},{onConflict:"project_id,module_id"});
      link.active=true;link.order_index=order;activeLinks.push(link);
    }

    const base=`${safeStructureCode(project.code||project.name,"PROJ")}-${safeStructureCode(room.code||room.name,"SALA")}`;
    let sequence=1;
    while(activeLinks.length<desired){
      let code;
      do{code=`${base}-M${String(sequence++).padStart(2,"0")}`}while(modules.some(row=>normalizeText(row.code)===normalizeText(code)));
      const displayName=`Módulo ${String(activeLinks.length+1).padStart(2,"0")}`;
      const{data:module,error:moduleError}=await sb.from("modules").insert({code,name:displayName,usage_suggested:"Módulo criado automaticamente pela quantidade definida na sala."}).select("*").single();
      if(moduleError)throw moduleError;
      modules.push(module);
      const order=nextOrder++;
      const{error:linkError}=await sb.from("project_room_modules").insert({project_id:projectId,room_id:roomId,module_id:module.id,display_name:displayName,order_index:order,active:true});
      if(linkError)throw linkError;
      await sb.from("project_modules").upsert({project_id:projectId,module_id:module.id,display_name:displayName,order_index:order,active:true},{onConflict:"project_id,module_id"});
      activeLinks.push({project_id:projectId,room_id:roomId,module_id:module.id,display_name:displayName,order_index:order,active:true});
    }

    if(activeLinks.length>desired){
      const keep=activeLinks.slice(0,desired);
      const remove=activeLinks.slice(desired);
      for(const link of remove){
        const{error}=await sb.from("project_room_modules").update({active:false})
          .eq("project_id",projectId).eq("room_id",roomId).eq("module_id",link.module_id);
        if(error)throw error;
        const usedElsewhere=projectRoomModules.some(other=>
          other.project_id===projectId&&other.module_id===link.module_id&&other.active&&other.room_id!==roomId
        );
        if(!usedElsewhere){
          await sb.from("project_modules").update({active:false}).eq("project_id",projectId).eq("module_id",link.module_id);
        }
      }
    }
  }
  ...
  function standardRoomDefinitionFor(room){
    if(!room)return null;
    const code=compactCatalogKey(room.code);
    const name=compactCatalogKey(room.name);
    return STANDARD_PROJECT_ROOMS.find(def=>
      def.aliases.includes(code)||def.aliases.includes(name)
    )||null;
  }

  function roomForStandardDefinition(def){
    const activeFirst=rooms.slice().sort((a,b)=>Number(b.active)-Number(a.active));
    return activeFirst.find(room=>standardRoomDefinitionFor(room)?.key===def.key)||null;
  }

  function projectCompositionMode(projectId){
    const activeLinks=projectRooms.filter(row=>row.project_id===projectId&&row.active);
    const hasMonoblock=activeLinks.some(link=>standardRoomDefinitionFor(rooms.find(room=>room.id===link.room_id))?.monoblock);
    if(hasMonoblock)return "Monobloco";
    if(activeLinks.length)return "Salas modulares";
    return "Não configurado";
  }

  function renderProjectComposition(){
    const projectSelect=$("projectCompositionProject");
    const container=$("projectCompositionRooms");
    const status=$("projectCompositionStatus");
    if(!projectSelect||!container)return;

    const projectId=projectSelect.value;
    if(!projectId){
      container.innerHTML='<div class="composition-empty">Selecione um projeto para definir sua composição.</div>';
      if(status)status.textContent="Selecione um projeto para configurar.";
      return;
    }

    const mode=projectCompositionMode(projectId);
    if(status)status.innerHTML=`<strong>${esc(projectName(projectId))}</strong><span>${esc(mode)}</span>`;

    const activeLinks=projectRooms.filter(row=>row.project_id===projectId&&row.active);
    const hasActiveMonoblock=activeLinks.some(link=>standardRoomDefinitionFor(rooms.find(room=>room.id===link.room_id))?.monoblock);

    container.innerHTML=STANDARD_PROJECT_ROOMS.map(def=>{
      const room=roomForStandardDefinition(def);
      const link=room?projectRooms.find(row=>row.project_id===projectId&&row.room_id===room.id):null;
      const checked=Boolean(link?.active);
      const activeCount=room?activeProjectRoomModuleCount(projectId,room.id):0;
      const quantity=Math.max(1,activeCount||1);
      const unavailable=!room;
      return `<article class="room-composition-card ${checked?"selected":""} ${def.monoblock?"monoblock-card":""} ${unavailable?"unavailable":""}" data-room-key="${def.key}">
        <div class="room-composition-card-head">
          <label class="room-enable-control">
            <input class="composition-room-enabled" type="checkbox" data-room-id="${room?.id||""}" data-monoblock="${def.monoblock?"true":"false"}" ${checked?"checked":""} ${unavailable?"disabled":""}>
            <span class="room-code-chip">${esc(def.short)}</span>
            <span><strong>${esc(def.title)}</strong><small>${def.monoblock?"Projeto monobloco — até 4 módulos":"Sala modular do projeto"}</small></span>
          </label>
        </div>
        ${unavailable
          ? `<p class="room-missing-warning">Cadastre esta sala na subaba Referências.</p>`
          : `<div class="room-module-quantity">
              <span>Quantidade de módulos</span>
              <div class="module-quantity-editor">
                <button class="module-quantity-button" type="button" data-module-quantity-change="-1" aria-label="Diminuir quantidade">−</button>
                <input
                  class="composition-room-count"
                  type="number"
                  min="1"
                  ${def.monoblock?`max="${MAX_MONOBLOCK_MODULES}"`:""}
                  step="1"
                  inputmode="numeric"
                  value="${quantity}"
                  ${checked?"":"disabled"}>
                <button class="module-quantity-button" type="button" data-module-quantity-change="1" aria-label="Aumentar quantidade">+</button>
              </div>
              <small>${def.monoblock?"Permitido de 1 a 4 módulos.":"Defina conforme a composição deste projeto."}</small>
            </div>`}
      </article>`;
    }).join("");

    if(hasActiveMonoblock){
      container.querySelectorAll('.room-composition-card:not(.monoblock-card)').forEach(card=>card.classList.add('muted-by-mode'));
    }
  }

  function enforceCompositionSelection(changedInput){
    const container=$("projectCompositionRooms");
    if(!container)return;
    const isMonoblock=changedInput.dataset.monoblock==="true";
    const checked=changedInput.checked;

    if(checked&&isMonoblock){
      container.querySelectorAll('.composition-room-enabled').forEach(input=>{
        if(input!==changedInput){input.checked=false;const select=input.closest('.room-composition-card')?.querySelector('.composition-room-count');if(select)select.disabled=true;}
      });
    }else if(checked){
      const mono=container.querySelector('.composition-room-enabled[data-monoblock="true"]');
      if(mono)mono.checked=false;
    }

    container.querySelectorAll('.room-composition-card').forEach(card=>{
      const input=card.querySelector('.composition-room-enabled');
      const select=card.querySelector('.composition-room-count');
      card.classList.toggle('selected',Boolean(input?.checked));
      if(select)select.disabled=!input?.checked;
      card.classList.remove('muted-by-mode');
    });

    const monoChecked=container.querySelector('.composition-room-enabled[data-monoblock="true"]:checked');
    if(monoChecked){
      container.querySelectorAll('.room-composition-card:not(.monoblock-card)').forEach(card=>card.classList.add('muted-by-mode'));
    }
  }

  function selectedStructureProjectId(){
    return $("projectCompositionProject")?.value||"";
  }

  function renderProjectStructureTables(){
    const admin=isAdmin();
    const selectedProject=selectedStructureProjectId();
    const roomRows=projectRooms
      .filter(link=>!selectedProject||link.project_id===selectedProject)
      .slice().sort((a,b)=>(a.order_index||0)-(b.order_index||0)||roomName(a.room_id,a.project_id).localeCompare(roomName(b.room_id,b.project_id),"pt-BR"));
    const moduleRows=projectRoomModules
      .filter(link=>!selectedProject||link.project_id===selectedProject)
      .slice().sort((a,b)=>roomName(a.room_id,a.project_id).localeCompare(roomName(b.room_id,b.project_id),"pt-BR")||(a.order_index||0)-(b.order_index||0));

    if($("projectRoomsCount"))$("projectRoomsCount").textContent=`${roomRows.filter(row=>row.active).length} sala${roomRows.filter(row=>row.active).length===1?"":"s"}`;
    if($("projectRoomsTable"))$("projectRoomsTable").innerHTML=roomRows.map(link=>`<tr>
      <td>${esc(projectName(link.project_id))}</td>
      <td>${esc(roomName(link.room_id,link.project_id))}</td>
      <td><strong>${activeProjectRoomModuleCount(link.project_id,link.room_id)}</strong></td>
      <td><span class="badge">${link.active?"Ativa":"Inativa"}</span></td>
      <td><div class="table-actions">${admin?`<button class="btn primary small" data-edit-project-room-project="${link.project_id}" data-edit-project-room-room="${link.room_id}">Editar</button>`:""}<button class="btn secondary small" data-toggle-project-room-project="${link.project_id}" data-toggle-project-room-room="${link.room_id}" data-active="${link.active}">${link.active?"Inativar":"Ativar"}</button>${admin?`<button class="btn danger small" data-delete-project-room-project="${link.project_id}" data-delete-project-room-room="${link.room_id}">Excluir</button>`:""}</div></td>
    </tr>`).join("")||'<tr><td colspan="5" class="empty">Nenhuma sala vinculada ao projeto selecionado.</td></tr>';

    if($("projectRoomModulesCount"))$("projectRoomModulesCount").textContent=`${moduleRows.filter(row=>row.active).length} módulo${moduleRows.filter(row=>row.active).length===1?"":"s"}`;
    if($("projectRoomModulesTable"))$("projectRoomModulesTable").innerHTML=moduleRows.map(link=>{
      const module=modules.find(row=>row.id===link.module_id);
      return `<tr>
        <td>${esc(projectName(link.project_id))}</td>
        <td>${esc(roomName(link.room_id,link.project_id))}</td>
        <td>${esc(module?.code||"—")}</td>
        <td>${esc(moduleName(link.module_id,link.project_id,link.room_id))}</td>
        <td>${Number(link.order_index||0)}</td>
        <td><span class="badge">${link.active?"Ativo":"Inativo"}</span></td>
        <td><div class="table-actions">${admin?`<button class="btn primary small" data-edit-project-room-module-project="${link.project_id}" data-edit-project-room-module-room="${link.room_id}" data-edit-project-room-module-module="${link.module_id}">Editar</button>`:""}<button class="btn secondary small" data-toggle-project-room-module-project="${link.project_id}" data-toggle-project-room-module-room="${link.room_id}" data-toggle-project-room-module-module="${link.module_id}" data-active="${link.active}">${link.active?"Inativar":"Ativar"}</button>${admin?`<button class="btn danger small" data-delete-project-room-module-project="${link.project_id}" data-delete-project-room-module-room="${link.room_id}" data-delete-project-room-module-module="${link.module_id}">Excluir</button>`:""}</div></td>
      </tr>`;
    }).join("")||'<tr><td colspan="7" class="empty">Nenhum módulo vinculado às salas do projeto selecionado.</td></tr>';
  }

  async function saveProjectComposition(){
    const projectId=selectedStructureProjectId();
    if(!projectId)return toast("Selecione o projeto.",true);
    const cards=[...$("projectCompositionRooms").querySelectorAll('.room-composition-card')];
    const selected=cards.filter(card=>card.querySelector('.composition-room-enabled')?.checked);
    if(!selected.length)return toast("Selecione pelo menos uma sala ou o Monobloco.",true);
    if(selected.some(card=>card.querySelector('[data-monoblock="true"]'))&&selected.length>1){
      return toast("O Monobloco não pode ser combinado com outras salas.",true);
    }

    showLoading(true);
    try{
      for(const def of STANDARD_PROJECT_ROOMS){
        const card=cards.find(item=>item.dataset.roomKey===def.key);
        const input=card?.querySelector('.composition-room-enabled');
        const roomId=input?.dataset.roomId;
        if(!roomId)continue;
        const existing=projectRooms.find(row=>row.project_id===projectId&&row.room_id===roomId);
        if(input.checked){
          const room=rooms.find(row=>row.id===roomId);
          const quantityInput=card.querySelector('.composition-room-count');
          const quantity=Number(quantityInput?.value||1);
          if(!Number.isInteger(quantity)||quantity<1){
            throw new Error(`Informe uma quantidade inteira maior que zero para ${def.title}.`);
          }
          if(def.monoblock&&quantity>MAX_MONOBLOCK_MODULES){
            throw new Error(`O MONOBLOCO permite no máximo ${MAX_MONOBLOCK_MODULES} módulos.`);
          }
          const{error}=await sb.from("project_rooms").upsert({
            project_id:projectId,
            room_id:roomId,
            display_name:room?.name||def.title,
            order_index:def.order,
            active:true
          },{onConflict:"project_id,room_id"});
          if(error)throw error;
          await syncProjectRoomModuleQuantity(projectId,roomId,quantity);
        }else if(existing?.active){
          const{error}=await sb.from("project_rooms").update({active:false}).eq("project_id",projectId).eq("room_id",roomId);
          if(error)throw error;
          await syncProjectRoomModuleQuantity(projectId,roomId,0);
        }
      }
      toast("Composição do projeto atualizada.");
      await reloadCatalogs();
    }catch(error){
      handleError(error,"Não foi possível salvar a composição do projeto.");
    }finally{
      showLoading(false);
    }
  }

  async function deleteProjectRoomLink(projectId,roomId){
    const links=projectRoomModules.filter(row=>row.project_id===projectId&&row.room_id===roomId);
    const moduleIds=links.map(row=>row.module_id);
    const{error:moduleLinksError}=await sb.from("project_room_modules").delete().eq("project_id",projectId).eq("room_id",roomId);
    if(moduleLinksError)throw moduleLinksError;
    const{error:roomLinkError}=await sb.from("project_rooms").delete().eq("project_id",projectId).eq("room_id",roomId);
    if(roomLinkError)throw roomLinkError;
    for(const moduleId of moduleIds){
      const usedInAnotherRoom=projectRoomModules.some(row=>row.project_id===projectId&&row.room_id!==roomId&&row.module_id===moduleId&&row.active);
      if(!usedInAnotherRoom){
        await sb.from("project_modules").update({active:false}).eq("project_id",projectId).eq("module_id",moduleId);
      }
    }
  }

  async function deleteProjectRoomModuleLink(projectId,roomId,moduleId){
    const{error}=await sb.from("project_room_modules").delete().eq("project_id",projectId).eq("room_id",roomId).eq("module_id",moduleId);
    if(error)throw error;
    const usedElsewhere=projectRoomModules.some(row=>row.project_id===projectId&&row.room_id!==roomId&&row.module_id===moduleId&&row.active);
    if(!usedElsewhere){
      await sb.from("project_modules").update({active:false}).eq("project_id",projectId).eq("module_id",moduleId);
    }
  }
  ...
  $("activitiesTable").onclick=async e=>{
    ...
    if(editId){
      ...
      $("editActivityId").value=x.id;$("editActivityCode").value=x.code||"";$("editActivityName").value=x.name;$("editActivityDiscipline").value=x.discipline_name||"";$("editActivityNature").value=x.nature||"";$("editActivityObservationRequirement").value=x.observation_requirement||"Recomendado";$("editActivityUsageDescription").value=x.usage_description||"";$("editActivityActive").value=String(x.active);
  ...
  $("saveActivityEditBtn").onclick=async e=>{
    ...
    try{
      ...
      const{error}=await sb.from("activities").update({code:$("editActivityCode").value.trim()||null,name:$("editActivityName").value.trim(),discipline_name:$("editActivityDiscipline").value.trim(),nature:$("editActivityNature").value.trim(),usage_description:$("editActivityUsageDescription").value.trim(),observation_requirement:$("editActivityObservationRequirement").value,sector_principal:areas.map(areaName).join(" / "),active:$("editActivityActive").value==="true"}).eq("id",id);
      if(error)throw error;
  ...
  $("projectCompositionRooms").onchange=e=>{
    const input=e.target.closest('.composition-room-enabled');
    if(input)enforceCompositionSelection(input);

    const quantityInput=e.target.closest('.composition-room-count');
    if(quantityInput){
      const maximum=Number(quantityInput.max||0);
      let value=Math.max(1,Math.floor(Number(quantityInput.value)||1));
      if(maximum>0)value=Math.min(maximum,value);
      quantityInput.value=String(value);
    }
  };

  $("projectCompositionRooms").onclick=e=>{
    const button=e.target.closest("[data-module-quantity-change]");
    if(!button)return;
    const editor=button.closest(".module-quantity-editor");
    const input=editor?.querySelector(".composition-room-count");
    if(!input||input.disabled)return;
    const change=Number(button.dataset.moduleQuantityChange||0);
    const current=Math.max(1,Math.floor(Number(input.value)||1));
    const maximum=Number(input.max||0);
    let next=Math.max(1,current+change);
    if(maximum>0)next=Math.min(maximum,next);
    input.value=String(next);
    input.dispatchEvent(new Event("change",{bubbles:true}));
  };

  $("saveProjectCompositionBtn").onclick=saveProjectComposition;

  $("projectRoomModuleProject").onchange=refreshProjectRoomModuleRoomSelect;

  $("projectRoomModuleForm").onsubmit=async e=>{
    e.preventDefault();
    showLoading(true);
    try{
      const projectId=$("projectRoomModuleProject").value;
      const roomId=$("projectRoomModuleRoom").value;
      const code=$("projectRoomModuleCode").value.trim().toUpperCase();
      const name=$("projectRoomModuleName").value.trim();
      const roomDefinition=standardRoomDefinitionFor(rooms.find(row=>row.id===roomId));
      const currentCount=activeProjectRoomModuleCount(projectId,roomId);
      const alreadyLinked=projectRoomModules.some(row=>row.project_id===projectId&&row.room_id===roomId&&normalizeText(modules.find(module=>module.id===row.module_id)?.code)===normalizeText(code));
      if(roomDefinition?.monoblock&&!alreadyLinked&&currentCount>=MAX_MONOBLOCK_MODULES){
        throw new Error(`O MONOBLOCO permite no máximo ${MAX_MONOBLOCK_MODULES} módulos.`);
      }

      let module=modules.find(row=>normalizeText(row.code)===normalizeText(code));
      if(!module){
        const{data,error}=await sb.from("modules").insert({
          code,
          name,
          usage_suggested:"Módulo vinculado a uma sala do projeto."
        }).select("*").single();
        if(error)throw error;
        module=data;
      }

      const{error:linkError}=await sb.from("project_room_modules").upsert({
        project_id:projectId,
        room_id:roomId,
        module_id:module.id,
        display_name:name,
        order_index:Number($("projectRoomModuleOrder").value||0),
        active:true
      },{onConflict:"project_id,room_id,module_id"});
      if(linkError)throw linkError;

      await sb.from("project_modules").upsert({
        project_id:projectId,
        module_id:module.id,
        display_name:name,
        order_index:Number($("projectRoomModuleOrder").value||0),
      ...
      $("projectRoomModuleForm").reset();
      toast("Módulo vinculado à sala.");
      await reloadCatalogs();
    }catch(error){handleError(error)}finally{showLoading(false)}
  };
  ...
  $("roomsTable").onclick=async e=>{
    ...
    if(deleteId){
      if(projectRooms.some(row=>row.room_id===deleteId))return toast("Remova primeiro a sala dos projetos na subaba Composição do projeto.",true);
  ...
  $("projectRoomsTable").onclick=async e=>{
    const button=e.target.closest("button");if(!button)return;
    const editProjectId=button.dataset.editProjectRoomProject;
    const editRoomId=button.dataset.editProjectRoomRoom;
    const deleteProjectId=button.dataset.deleteProjectRoomProject;
    const deleteRoomId=button.dataset.deleteProjectRoomRoom;
    if(editProjectId&&editRoomId){
      const link=projectRooms.find(row=>row.project_id===editProjectId&&row.room_id===editRoomId);if(!link)return;
      $("editProjectRoomProjectId").value=editProjectId;$("editProjectRoomRoomId").value=editRoomId;
      $("editProjectRoomProjectName").value=projectName(editProjectId);$("editProjectRoomCatalogName").value=rooms.find(row=>row.id===editRoomId)?.name||roomName(editRoomId,editProjectId);
      $("editProjectRoomDisplayName").value=link.display_name||roomName(editRoomId,editProjectId);$("editProjectRoomModuleQuantity").value=String(activeProjectRoomModuleCount(editProjectId,editRoomId));$("editProjectRoomOrder").value=String(link.order_index||0);$("editProjectRoomActive").value=String(link.active);$("editProjectRoomDialog").showModal();return;
    }
    if(deleteProjectId&&deleteRoomId){
      if(!confirm(`Excluir ${roomName(deleteRoomId,deleteProjectId)} da estrutura de ${projectName(deleteProjectId)}?`))return;
      showLoading(true);
      try{await deleteProjectRoomLink(deleteProjectId,deleteRoomId);toast("Sala removida do projeto.");await reloadCatalogs()}catch(error){handleError(error,"Não foi possível excluir a sala do projeto.")}finally{showLoading(false)}
      return;
    }
    const projectId=button.dataset.toggleProjectRoomProject;const roomId=button.dataset.toggleProjectRoomRoom;
    if(!projectId||!roomId)return;
    const{error}=await sb.from("project_rooms").update({active:button.dataset.active!=="true"}).eq("project_id",projectId).eq("room_id",roomId);
    if(error)handleError(error);else reloadCatalogs();
  };

  $("saveProjectRoomEditBtn").onclick=async e=>{e.preventDefault();showLoading(true);try{
    const projectId=$("editProjectRoomProjectId").value;const roomId=$("editProjectRoomRoomId").value;
    const{error}=await sb.from("project_rooms").update({display_name:$("editProjectRoomDisplayName").value.trim(),order_index:Number($("editProjectRoomOrder").value||0),active:$("editProjectRoomActive").value==="true"}).eq("project_id",projectId).eq("room_id",roomId);if(error)throw error;
    const roomDefinition=standardRoomDefinitionFor(rooms.find(row=>row.id===roomId));
    const quantityValue=Number($("editProjectRoomModuleQuantity").value||1);
    if(!Number.isInteger(quantityValue)||quantityValue<1){
      throw new Error("A quantidade de módulos deve ser um número inteiro maior que zero.");
    }
    if(roomDefinition?.monoblock&&quantityValue>MAX_MONOBLOCK_MODULES){
      throw new Error(`O MONOBLOCO permite no máximo ${MAX_MONOBLOCK_MODULES} módulos.`);
    }
    const desiredQuantity=quantityValue;
    await syncProjectRoomModuleQuantity(projectId,roomId,desiredQuantity);
    $("editProjectRoomDialog").close();toast("Sala do projeto atualizada.");await reloadCatalogs();
  }catch(error){handleError(error)}finally{showLoading(false)}};

  $("projectRoomModulesTable").onclick=async e=>{
    const button=e.target.closest("button");if(!button)return;
    const editProjectId=button.dataset.editProjectRoomModuleProject;const editRoomId=button.dataset.editProjectRoomModuleRoom;const editModuleId=button.dataset.editProjectRoomModuleModule;
    const deleteProjectId=button.dataset.deleteProjectRoomModuleProject;const deleteRoomId=button.dataset.deleteProjectRoomModuleRoom;const deleteModuleId=button.dataset.deleteProjectRoomModuleModule;
    if(editProjectId&&editRoomId&&editModuleId){
      const link=projectRoomModules.find(row=>row.project_id===editProjectId&&row.room_id===editRoomId&&row.module_id===editModuleId);const module=modules.find(row=>row.id===editModuleId);if(!link||!module)return;
      $("editProjectRoomModuleProjectId").value=editProjectId;$("editProjectRoomModuleRoomId").value=editRoomId;$("editProjectRoomModuleModuleId").value=editModuleId;
      $("editProjectRoomModuleProjectName").value=projectName(editProjectId);$("editProjectRoomModuleRoomName").value=roomName(editRoomId,editProjectId);$("editProjectRoomModuleCode").value=module.code||"";$("editProjectRoomModuleDisplayName").value=link.display_name||module.name||"";$("editProjectRoomModuleOrder").value=String(link.order_index||0);$("editProjectRoomModuleActive").value=String(link.active);$("editProjectRoomModuleDialog").showModal();return;
    }
    if(deleteProjectId&&deleteRoomId&&deleteModuleId){
      if(!confirm(`Excluir ${moduleName(deleteModuleId,deleteProjectId,deleteRoomId)} desta sala?`))return;
      showLoading(true);
      try{await deleteProjectRoomModuleLink(deleteProjectId,deleteRoomId,deleteModuleId);toast("Módulo removido da sala.");await reloadCatalogs()}catch(error){handleError(error,"Não foi possível excluir o módulo da sala.")}finally{showLoading(false)}
      return;
    }
    const projectId=button.dataset.toggleProjectRoomModuleProject;const roomId=button.dataset.toggleProjectRoomModuleRoom;const moduleId=button.dataset.toggleProjectRoomModuleModule;
    if(!projectId||!roomId||!moduleId)return;
    const{error}=await sb.from("project_room_modules").update({active:button.dataset.active!=="true"}).eq("project_id",projectId).eq("room_id",roomId).eq("module_id",moduleId);
    if(error)handleError(error);else reloadCatalogs();
  };

  $("saveProjectRoomModuleEditBtn").onclick=async e=>{e.preventDefault();showLoading(true);try{
    const projectId=$("editProjectRoomModuleProjectId").value;const roomId=$("editProjectRoomModuleRoomId").value;const moduleId=$("editProjectRoomModuleModuleId").value;const displayName=$("editProjectRoomModuleDisplayName").value.trim();const order=Number($("editProjectRoomModuleOrder").value||0);const active=$("editProjectRoomModuleActive").value==="true";
    const{error:moduleError}=await sb.from("modules").update({code:$("editProjectRoomModuleCode").value.trim().toUpperCase(),name:displayName,active}).eq("id",moduleId);if(moduleError)throw moduleError;
    const{error:linkError}=await sb.from("project_room_modules").update({display_name:displayName,order_index:order,active}).eq("project_id",projectId).eq("room_id",roomId).eq("module_id",moduleId);if(linkError)throw linkError;
    await sb.from("project_modules").upsert({project_id:projectId,module_id:moduleId,display_name:displayName,order_index:order,active},{onConflict:"project_id,module_id"});
    $("editProjectRoomModuleDialog").close();toast("Módulo atualizado.");await reloadCatalogs();
  }catch(error){handleError(error)}finally{showLoading(false)}};
      ...
      if ($("importCatalogs").checked || $("importEntries").checked) {
        ...
        if(importedActivityIds.length){
          ...
          if(missingLinks.length){
            const{error:areaLinkError}=await sb.from("activity_area_links").insert(missingLinks.map(activity_id=>({activity_id,area_code:"ADM"})));
  ...
  window.APONTA_P3_VERSION = "2.15.2";
})();

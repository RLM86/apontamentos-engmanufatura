(() => {
  ...
  function recoveryErrorDetails(error) {
    ...
    return {
      ...
      code,
      message,
      normalized: normalizeText(`${code} ${message}`)
    };
  }
  ...
  function recoveryErrorMessage(error) {
    ...
    if (details.message) {
      return `Não foi possível enviar o link: ${details.message}`;
    }

    const technicalCode = details.code || (
      details.status ? `HTTP ${details.status}` : "AUTH-RECOVERY-UNKNOWN"
    );

    return (
      "Não foi possível enviar o link de redefinição. " +
      `Código: ${technicalCode}. ` +
      "Confira o limite de e-mails e a configuração SMTP do Supabase."
    );
  }
...
`+
  ...
  if("serviceWorker" in navigator && location.protocol.startsWith("http")){
    navigator.serviceWorker
      .register("sw.js?v=2.19.4", {updateViaCache:"none"})
  ...
  window.APONTA_P3_VERSION = "2.19.4";
})();

(() => {
  ...
  async function renderCatalogs(){
    ...
    $("sectorsTable").innerHTML=manufacturingSectors.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.active?"Ativo":"Inativo"}</td><td><button class="btn secondary small" data-toggle-sector="${x.id}" data-active="${x.active}">${x.active?"Inativar":"Ativar"}</button></td></tr>`).join("");
    $("panelTypesTable").innerHTML=panelTypes.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.active?"Ativo":"Inativo"}</td><td><button class="btn secondary small" data-toggle-panel="${x.id}" data-active="${x.active}">${x.active?"Inativar":"Ativar"}</button></td></tr>`).join("");
    $("roomsTable").innerHTML=rooms.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.active?"Ativa":"Inativa"}</td><td><button class="btn secondary small" data-toggle-room="${x.id}" data-active="${x.active}">${x.active?"Inativar":"Ativar"}</button></td></tr>`).join("");

    $("projectRoomsCount").textContent=`${projectRooms.length} vínculos`;
    $("projectRoomsTable").innerHTML=projectRooms
      .slice()
      .sort((a,b)=>projectName(a.project_id).localeCompare(projectName(b.project_id),"pt-BR")||(a.order_index||0)-(b.order_index||0))
      .map(link=>`<tr>
        <td>${esc(projectName(link.project_id))}</td>
        <td>${esc(roomName(link.room_id,link.project_id))}</td>
        <td>${link.active?"Ativa":"Inativa"}</td>
        <td><button class="btn secondary small"
          data-toggle-project-room-project="${link.project_id}"
          data-toggle-project-room-room="${link.room_id}"
          data-active="${link.active}">
          ${link.active?"Inativar":"Ativar"}
        </button></td>
      </tr>`).join("")||'<tr><td colspan="4" class="empty">Nenhuma sala vinculada aos projetos</td></tr>';

    $("projectRoomModulesCount").textContent=`${projectRoomModules.length} vínculos`;
    $("projectRoomModulesTable").innerHTML=projectRoomModules
      .slice()
      .sort((a,b)=>projectName(a.project_id).localeCompare(projectName(b.project_id),"pt-BR")||
        roomName(a.room_id,a.project_id).localeCompare(roomName(b.room_id,b.project_id),"pt-BR")||
      ...
      .map(link=>`<tr>
        ...
        <td><button class="btn secondary small"
          data-toggle-project-room-module-project="${link.project_id}"
          data-toggle-project-room-module-room="${link.room_id}"
          data-toggle-project-room-module-module="${link.module_id}"
          data-active="${link.active}">
          ${link.active?"Inativar":"Ativar"}
        </button></td>
      </tr>`).join("")||'<tr><td colspan="5" class="empty">Nenhum módulo vinculado às salas</td></tr>';
  ...
  $("projectRoomForm").onsubmit=async e=>{
    ...
    try{
      ...
      const{error}=await sb.from("project_rooms").upsert({
        ...
        order_index:Number($("projectRoomOrder").value||0),
        active:true
      },{onConflict:"project_id,room_id"});
      if(error)throw error;
      $("projectRoomForm").reset();
      toast("Sala vinculada ao projeto.");
  ...
  $("sectorsTable").onclick=async e=>{const id=e.target.dataset.toggleSector;if(!id)return;const{error}=await sb.from("manufacturing_sectors").update({active:e.target.dataset.active!=="true"}).eq("id",id);if(error)handleError(error);else reloadCatalogs()};
  $("panelTypesTable").onclick=async e=>{const id=e.target.dataset.togglePanel;if(!id)return;const{error}=await sb.from("panel_types").update({active:e.target.dataset.active!=="true"}).eq("id",id);if(error)handleError(error);else reloadCatalogs()};
  $("roomsTable").onclick=async e=>{const id=e.target.dataset.toggleRoom;if(!id)return;const{error}=await sb.from("rooms").update({active:e.target.dataset.active!=="true"}).eq("id",id);if(error)handleError(error);else reloadCatalogs()};

  $("projectRoomsTable").onclick=async e=>{
    const projectId=e.target.dataset.toggleProjectRoomProject;
    const roomId=e.target.dataset.toggleProjectRoomRoom;
    if(!projectId||!roomId)return;
    const{error}=await sb.from("project_rooms")
      .update({active:e.target.dataset.active!=="true"})
  ...
  $("projectRoomModulesTable").onclick=async e=>{
    const projectId=e.target.dataset.toggleProjectRoomModuleProject;
    const roomId=e.target.dataset.toggleProjectRoomModuleRoom;
    const moduleId=e.target.dataset.toggleProjectRoomModuleModule;
    if(!projectId||!roomId||!moduleId)return;
    const{error}=await sb.from("project_room_modules")
      .update({active:e.target.dataset.active!=="true"})
      .eq("project_id",projectId)
      .eq("room_id",roomId)
  ...
  window.APONTA_P3_VERSION = "2.13.0";
})();

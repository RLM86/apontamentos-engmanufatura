(() => {
  ...
  function setEntryStepNumbers(prefix, areaCode) {
    ...
    if($("entryActivityStep"))$("entryActivityStep").textContent=structural?"6":noDetail?"3":"4";
  }
  ...
  function entryFlowPayload(prefix) {
    ...
    return {
      ...
      module_part:areaCode==="MES"?$(prefix+"ModulePart").value:null
    };
  }
  ...
  function fillProjectStructureSelects() {
    ...
    ["projectCompositionProject","projectRoomModuleProject"].forEach(id=>{const select=$(id);if(!select)return;const old=select.value;select.innerHTML=projectOptions;if(old&&activeRows(projects).some(row=>row.id===old))select.value=old;});
    const composition=$("projectCompositionProject");if(composition&&!composition.value&&activeRows(projects).length)composition.value=activeRows(projects)[0].id;
    if($("projectRoomModuleProject")&&composition?.value)$("projectRoomModuleProject").value=composition.value;
    const roomCatalog=$("projectRoomInstanceRoom");if(roomCatalog){const old=roomCatalog.value;roomCatalog.innerHTML=optionsWithPlaceholder(activeRows(rooms),row=>`${row.code?row.code+" — ":""}${row.name}`,row=>row.id,"Selecione o tipo de sala");if(old)roomCatalog.value=old;}
    refreshProjectRoomModuleRoomSelect();renderProjectComposition();renderProjectStructureTables();
  }
  ...
  function refreshProjectRoomModuleRoomSelect() {
    ...
    roomSelect.innerHTML=optionsWithPlaceholder(rows,row=>row.name,row=>row.id,projectSelect.value?"Selecione a sala específica":"Selecione primeiro o projeto");roomSelect.disabled=!projectSelect.value;if(old&&rows.some(row=>row.id===old))roomSelect.value=old;
  }
  ...
  async function syncRoomInstanceModuleQuantity(instanceId,desiredValue){
    ...
    while(active.length<desired&&inactive.length){const row=inactive.shift();const{error}=await sb.from("project_room_instance_modules").update({active:true,updated_at:new Date().toISOString()}).eq("id",row.id);if(error)throw error;row.active=true;active.push(row);}
    let next=Math.max(0,...rows.map(row=>Number(row.module_number||0)))+1;
    while(active.length<desired){const number=next++;const code=`${safeStructureCode(instance.code||instance.display_name,"SALA")}-M${String(number).padStart(2,"0")}`;const display=`Módulo ${String(number).padStart(2,"0")}`;const{data,error}=await sb.from("project_room_instance_modules").insert({room_instance_id:instanceId,module_number:number,code,display_name:display,order_index:number,has_lower_part:true,has_upper_part:true,active:true}).select("*").single();if(error)throw error;projectRoomInstanceModules.push(data);active.push(data);}
  ...
  function renderProjectStructureTables(){
    ...
    const roomRows=projectRoomInstances.filter(row=>!projectId||row.project_id===projectId).sort((a,b)=>(a.order_index||0)-(b.order_index||0)||(a.instance_number||0)-(b.instance_number||0));
    const moduleRows=projectRoomInstanceModules.filter(row=>{const instance=projectRoomInstances.find(item=>item.id===row.room_instance_id);return instance&&(!projectId||instance.project_id===projectId);}).sort((a,b)=>(a.order_index||0)-(b.order_index||0));
    ...
    Array.from(selectedProjectRoomIds).forEach(id=>{if(!existingRoomIds.has(id))selectedProjectRoomIds.delete(id)});
    $("projectRoomsTable").innerHTML=roomRows.map(row=>`<tr><td><input class="room-instance-selector" type="checkbox" data-select-room-instance="${row.id}" ${selectedProjectRoomIds.has(row.id)?"checked":""} aria-label="Selecionar ${esc(row.display_name||roomCatalogName(row.room_id))}"></td><td>${esc(projectName(row.project_id))}</td><td>${esc(roomCatalogCode(row.room_id))}</td><td>${esc(row.display_name||roomCatalogName(row.room_id))}</td><td>${row.instance_number}</td><td><strong>${instanceModuleCount(row.id)}</strong></td><td><span class="badge">${row.active?"Ativa":"Inativa"}</span></td><td><div class="table-actions"><button class="btn primary small" data-edit-room-instance="${row.id}">Editar</button><button class="btn secondary small" data-toggle-room-instance="${row.id}" data-active="${row.active}">${row.active?"Inativar":"Ativar"}</button><button class="btn danger small" data-delete-room-instance="${row.id}">Excluir</button></div></td></tr>`).join("")||'<tr><td colspan="8" class="empty">Nenhuma sala cadastrada para este projeto</td></tr>';
    ...
    $("projectRoomModulesTable").innerHTML=moduleRows.map(row=>{const instance=projectRoomInstances.find(item=>item.id===row.room_instance_id);const parts=[row.has_lower_part?"Inferior":"",row.has_upper_part?"Superior":""].filter(Boolean).join(" + ")||"Nenhuma";return `<tr><td>${esc(projectName(instance?.project_id))}</td><td>${esc(roomInstanceName(row.room_instance_id))}</td><td>${esc(row.code)}</td><td>${esc(row.display_name)}</td><td>${esc(parts)}</td><td><span class="badge">${row.active?"Ativo":"Inativo"}</span></td><td><div class="table-actions"><button class="btn primary small" data-edit-instance-module="${row.id}">Editar</button><button class="btn secondary small" data-toggle-instance-module="${row.id}" data-active="${row.active}">${row.active?"Inativar":"Ativar"}</button><button class="btn danger small" data-delete-instance-module="${row.id}">Excluir</button></div></td></tr>`;}).join("")||'<tr><td colspan="7" class="empty">Nenhum módulo cadastrado</td></tr>';
  }
  ...
  $("projectCompositionProject").onchange=()=>{const projectId=$("projectCompositionProject").value;if($("projectRoomModuleProject"))$("projectRoomModuleProject").value=projectId;refreshProjectRoomModuleRoomSelect();renderProjectComposition();renderProjectStructureTables();};
  ...
  $("projectRoomInstanceForm").onsubmit=async e=>{e.preventDefault();showLoading(true);try{
    ...
    toast(`${roomCount} sala${roomCount===1?"":"s"} adicionada${roomCount===1?"":"s"}.`);$("projectRoomInstanceForm").reset();$("projectRoomInstanceCount").value="1";$("projectRoomInstanceModuleCount").value="1";await reloadCatalogs();
  }catch(error){handleError(error)}finally{showLoading(false)}};

  $("projectRoomModuleProject").onchange=refreshProjectRoomModuleRoomSelect;
  $("projectRoomModuleForm").onsubmit=async e=>{e.preventDefault();showLoading(true);try{const instanceId=$("projectRoomModuleRoom").value;const instance=projectRoomInstances.find(row=>row.id===instanceId);if(!instance)throw new Error("Selecione a sala específica.");const number=Number($("projectRoomModuleOrder").value||1);const lower=$("projectRoomModuleLower").checked,upper=$("projectRoomModuleUpper").checked;if(!lower&&!upper)throw new Error("Habilite pelo menos uma parte do módulo.");const{error}=await sb.from("project_room_instance_modules").insert({room_instance_id:instanceId,module_number:number,code:$("projectRoomModuleCode").value.trim().toUpperCase(),display_name:$("projectRoomModuleName").value.trim(),order_index:number,has_lower_part:lower,has_upper_part:upper,active:true});if(error)throw error;$("projectRoomModuleForm").reset();$("projectRoomModuleLower").checked=true;$("projectRoomModuleUpper").checked=true;toast("Módulo adicionado.");await reloadCatalogs();}catch(error){handleError(error)}finally{showLoading(false)}};
  ...
  $("projectRoomModulesTable").onclick=async e=>{const button=e.target.closest("button");if(!button)return;const editId=button.dataset.editInstanceModule,toggleId=button.dataset.toggleInstanceModule,deleteId=button.dataset.deleteInstanceModule;
    if(editId){const row=projectRoomInstanceModules.find(item=>item.id===editId);const instance=projectRoomInstances.find(item=>item.id===row?.room_instance_id);if(!row||!instance)return;$("editProjectRoomInstanceModuleId").value=row.id;$("editProjectRoomModuleProjectName").value=projectName(instance.project_id);$("editProjectRoomModuleRoomName").value=roomInstanceName(instance.id);$("editProjectRoomModuleNumber").value=String(row.module_number);$("editProjectRoomModuleCode").value=row.code;$("editProjectRoomModuleDisplayName").value=row.display_name;$("editProjectRoomModuleOrder").value=String(row.order_index||0);$("editProjectRoomModuleLower").checked=row.has_lower_part!==false;$("editProjectRoomModuleUpper").checked=row.has_upper_part!==false;$("editProjectRoomModuleActive").value=String(row.active);$("editProjectRoomModuleDialog").showModal();return;}
  ...
  if("serviceWorker" in navigator && location.protocol.startsWith("http")){
    navigator.serviceWorker
      .register("sw.js?v=2.17.1", {updateViaCache:"none"})
  ...
  window.APONTA_P3_VERSION = "2.17.1";
})();
